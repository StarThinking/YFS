		...
		unsigned int xid_rep, char **b, int *sz)
{
	ScopedLock rwl(&reply_window_m_);
        ...
        std::map<unsigned int, std::list<reply_t> >::iterator list_it = reply_window_.find(clt_nonce);
        std::list<reply_t>::iterator reply_it;
        std::map<unsigned int, unsigned int>::iterator xid_it = xid_rep_map.find(clt_nonce);

        // update xid_rep map
        if(xid_it == xid_rep_map.end())
        {
            xid_rep_map.insert(std::pair<unsigned int, unsigned int>(clt_nonce, xid_rep));
            reply_t r(xid);
            list_it->second.insert(list_it->second.begin(), r);
            if(xid_rep > xid_it->second)
               xid_it->second = xid_rep;
            return NEW;
        }

        // delete
        for(reply_it=list_it->second.begin(); reply_it!=list_it->second.end(); )
        {
           if(reply_it->xid <= xid_rep)
               reply_it = list_it->second.erase(reply_it);
           else 
               reply_it++;
        }
        
        // find xid
        for(reply_it=list_it->second.begin(); reply_it!=list_it->second.end(); reply_it++)
        {
            if(reply_it->xid == xid)
               break;
        }
        ...
        // judge
        if(reply_it == list_it->second.end())
        {
            unsigned int last_xid_rep = xid_it->second;
            if(xid > last_xid_rep)
            {
                reply_t reply(xid);
                list_it->second.insert(list_it->second.begin(), reply);
                if(xid_rep > xid_it->second)
                    xid_it->second = xid_rep;
                return NEW;
            }
        ...
        {
           reply_t reply = *reply_it;
           if(reply.cb_present == true)
           {
              *b = reply.buf;
              *sz = reply.sz;
              if(xid_rep > xid_it->second)
		...
		char *b, int sz)
{
	ScopedLock rwl(&reply_window_m_);
        // You fill this in for Lab 1.
        std::map<unsigned int, std::list<reply_t> >::iterator list_it = reply_window_.find(clt_nonce);
        // update the reply  info and set true
