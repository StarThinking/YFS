...
main(int argc, char *argv[])
{
  ...
  if(pid == 0){
    ...
    printf("[msx-log] in the child thread, after createn\n");
    exit(0);
  }
  createn(d1, "zz", 20, true);
  printf("[msx-log] in the parent thread, after createn\n");
  sleep(4);
  dircheck(d1, 20);
  printf("[msx-log] in the parent thread, after dircheck(d1, 20)\n");
  reap(pid);
  printf("[msx-log] in the aprent thread, after reap(pid)\n");
  checkn(d1, "zz", 20);
  checkn(d2, "zz", 20);
  unlinkn(d1, "zz", 20);
  printf("OK\n");
