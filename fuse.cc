                   ...
                   int to_set, struct fuse_file_info *fi)
{
  ...
  if (FUSE_SET_ATTR_SIZE & to_set) {
    ...
    if(yfs->setattr(ino, attr->st_size))
    {
       getattr(ino, st);
       fuse_reply_attr(req, &st, 0);
    }
...
{
  printf("read file in fuse, ino = %016lx, size = %d, off = %lld\n", ino, size, off);
#if 1
  std::string buf;
  // Change the above "#if 0" to "#if 1", and your code goes here
  if(yfs->read(ino, size, off, buf))
  {
...
fuseserver_write(fuse_req_t req, fuse_ino_t ino,
                 ...
                 struct fuse_file_info *fi)
{
  printf("write file in fuse");
     
#if 1
  // Change the above line to "#if 1", and your code goes here
  if(yfs->write(ino, buf, size, off))
...
{
  printf("[msx-log] now in fuse create_help\n");
  fuse_ino_t ino = (fuse_ino_t)rand();
  printf("[msx-log] the random fileid = %016lx\n", ino);
  ino = ino | 0x80000000;
  printf("[msx-log] the random fileid after huo = %016lx\n", ino);
  printf("[msx-log] createhelper , parent = %016lx, name = %s, ino = %016lx\n", parent, name, ino);
  if(!yfs->create(parent, ino, name))
  {
     printf("[msx-log] create file error\n");
...
{
  printf("[msx-log] now in fuse lookup parent = %016lx and name = %s\n",parent, name );
  struct fuse_entry_param e;
  fuse_ino_t id;
  struct stat st;
  // In yfs, timeouts are always set to 0.0, and generations are always set to 0
  ...
  e.generation = 0;
  bool found = false;
  id = yfs->lookup(parent, name);
  if (id > 0)
  {
     printf("[msx-log] lookup and found id = %016lx\n", id);
     getattr(id, st);
     e.ino = id;
     e.attr = st;
     found = true;
  ...
  {
     printf("[msx-log] cannot find this name in the parent dir\n");
  }
  if (found)
    fuse_reply_entry(req, &e);
  else
                   ...
                   off_t off, struct fuse_file_info *fi)
{
  ...
  printf("[msx-log] now in fuse fuseserver_readdir\n");
  ...
  std::vector<unsigned long long> ino_list;
  if(yfs->readdir(ino, name_list, ino_list) == false)
     fuse_reply_err(req, ENOTDIR);
  for(int i=0; i<name_list.size(); i++)
  {
     dirbuf_add(&b, name_list[i].c_str(), ino_list[i]);
  }
...
fuseserver_mkdir(fuse_req_t req, fuse_ino_t parent, const char *name,
     mode_t mode)
{
  printf("[msx_log] now in fuseserver_mkdir\n");
  fuse_ino_t ino = (fuse_ino_t)rand();
  printf("[msx_log] the random dirid = %016lx\n", ino);
  ino = ino & 0x7fffffff;
  printf("[msx_log] the random dirid after yu= %016lx\n", ino);
  printf("[msx_log] mkdir : parent_ino = %016lx, dir_name = %s, dir_ino = %016lx\n",  parent, name, ino);
  if(!yfs->mkdir(parent, ino, name))
  {
     printf("[msx_log] failed to mkdir as same name existed\n");
     fuse_reply_err(req, EEXIST);
  }
  struct fuse_entry_param e;
  // In yfs, timeouts are always set to 0.0, and generations are always set to 0
...
fuseserver_unlink(fuse_req_t req, fuse_ino_t parent, const char *name)
{
  ...
  printf("[msx_log] now in fuseserver_unlink, parent = %016lx, name = %s\n",parent, name);
  fuse_ino_t id;
  id = yfs->lookup(parent, name);
  if(id <= 0)
  {
     printf("[msx_log] cannot find the name in the parent\n");
     fuse_reply_err(req, ENOENT);
  }
  else
  {
     if(yfs->isfile(id))
     {
        printf("[msx_log] the id found is a file , then unlink it, the id = %016lx\n", id);
        yfs->unlink(parent, id);
